...
def get_walks_n2v():
    ...
    #comment it because that make the file uncompilable when you use tensorflow 
...
def create_lstm(seqs, lstm_one_hot_list, L):
    ...
    #assigned the last result that returned in an interactive python session
    ...
    train_op = tf.train.AdamOptimizer(learning_rate=0.01).minimize(total_loss)  #adam optimizer
    ...
    weights = model_reader.get_tensor('rnn/multi_rnn_cell/cell_0/basic_lstm_cell/kernel')# shape of the layer 259*512
...
def create_initialize_lstm(seqs, lstm_one_hot_list, L, n2v_nodes, n2v_embs):
	...
	#state the initialize weight refer to n2v
    def getcell(emb_size):
        return cell_fun(emb_size)
	#weight replacement to the first layer, for here I find each network weight's name and state it
    with tf.variable_scope('change_weigths'):
            cell = tf.contrib.rnn.MultiRNNCell([getcell(emb_size) for _ in range(2)], state_is_tuple=True)
    ...
    array = np.random.rand(encoded_length, 4 * emb_size)  # shape of the layer 131*512
...
if __name__ == "__main__":
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = "0"#comment it if you use CPU
    #dir = 'data/dnc/test/'
    dir1 = './l10/'
    ...
    count = 0
    for i in range(args.firstFile, args.lastFile + 28):#make sure every graph node are acceptable
